...
if ( empty($_REQUEST['last']) ) {
	...
	echo "<script>jQuery('#contact_form .message').removeClass('d-none');jQuery('#contact_alert').removeClass('success').addClass('notice');</script>";
	echo '<h4>' . $errormessage . '</h4>';
	echo '<ul class="no-bottom">'.$alert.'</ul>';
	...
	// If the user didn't err and there is in fact a message, time to email it.
		...
		echo "<script>jQuery('#contact_form .message').removeClass('d-none');jQuery('#contact_alert').removeClass('notice').addClass('success');jQuery('#contact_form')[0].reset();</script>";
		echo $thanks;
